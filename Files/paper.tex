...
modelled by preferential attachment. The power law has been challenged
by the presence of alternatives with comparable performance, as well as
the recent findings that the tail of the degree distribution is often
lighter than implied by the body, whilst still being regularly varying.
We use methods from discrete extremes to study a version of the model
with a flexible preference function that allows super/sub-linear
behaviour whilst guaranteeing the limiting degree distribution is
regularly varying. We relate the tail index directly to the model
parameters and enable inference of parameters from the degree
distribution alone.}
