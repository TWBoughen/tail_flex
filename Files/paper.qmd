...
$$ {#eq-survlim}
for some characteristic function of the tree $\varphi(\cdot)$.

Using this result, and the characteristic function $\varphi(\Upsilon(t)) = \mathbb I\{\text{deg}(x,\Upsilon(t))>k\}$, we can write the limiting survival as:
...
\end{cases}
$$ {#eq-igp-est} meaning that for $k\ge k_0$ the limiting degree distribution (for large $k_0^\alpha$) is approximated by the $\text{IGP}\left(\frac{\beta}{\lambda^*}, \frac{k_0^\alpha + \varepsilon+\beta}{\lambda^*},k_0-1\right)$ distribution.
