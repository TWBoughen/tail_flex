...
As it is assumed throughout that $b(k)$ is non-decreasing, we consider the two cases where $b(k)$ is bounded and unbounded. When $b(k)$ is bounded, by the monotone convergence theorem, $\lim_{k\rightarrow\infty}b(k) = c$ for some $c>0$, which implies that $\lim_{k\rightarrow\infty}\Omega(F,k)= 0$. This proves the first part of @rem-omega. Now, using @eq-surv-origin,  $\frac{\bar{F}(k+1)}{\bar{F}(k)}=\frac{b(k+1)}{\lambda^{*}+b(k+1)}\rightarrow\frac{c}{\lambda^{*}+c}<1$ as $k\rightarrow\infty$. This means that $F$ is not long-tailed, and therefore is only recovered to the Gumbel MDA.
