...
To demonstrate that the model is capturing the tail behaviour of the
degree distributions Figure~\ref{fig-gp_comp} shows the tail heaviness
based on samples from the MCMC for each network as well as the tail
heaviness as estimated by fitting the GPD to the degrees for various
thresholds.
...
\includegraphics[width=0.8\textwidth,height=\textheight]{paper_files/figure-pdf/fig-gp_comp-1.pdf}

}

\caption{\label{fig-gp_comp}Posterior samples of shape parameter from
model (blue) and GPD shape parameter (red) at various thresholds}
