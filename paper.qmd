...
To demonstrate that the model is capturing the tail behaviour of the degree distributions @fig-gp_comp shows the tail heaviness based on samples from the MCMC for each network as well as the tail heaviness as estimated by fitting the GPD to the degrees for various thresholds.

```{r, echo=FALSE, warning=FALSE, cache=TRUE, out.width="80%"}
#| fig-width: 9
#| fig-height: 6
#| fig-cap: Posterior samples of shape parameter from model (blue) and GPD shape parameter (red) at various thresholds
#| label: fig-gp_comp

plt2 = readRDS('gp_plt.rds')
