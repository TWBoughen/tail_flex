...
igpd_rho = function(pars, shape, scale, threshold, exceedance){
  ...
  x = 1:(threshold+1)
  p1 = sum(cumprod(
    (x^pars[1] - (threshold+1)^pars[1] + pars[2]*scale)/
      (x^pars[1] - (threshold+1)^pars[1] + pars[2]*(scale+1))
  ))
  return(p1 + p2)
}
...
for(r in 1:nrow(close_pars)){
  if(r==1){
    plot(x, pref_mat[r,]/sum(pref_mat[r,]),type='l',log='xy')
  }else{
    lines(x, pref_mat[r,]/sum(pref_mat[r,]))
  }
}

real_pref = pref(x,a, eps, k0, b)

lines(x,real_pref/sum(real_pref), col='red')
